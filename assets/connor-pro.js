...
(() => {
  ...
  try {
    const bar = qs('#scrollProgress'); const topBtn = qs('#backToTop');
    const onScroll = () => {
      const doc = document.documentElement;
      const max = doc.scrollHeight - doc.clientHeight;
      const p = max>0 ? (doc.scrollTop / max) : 0;
      if (bar) bar.style.transform = `scaleX(${p})`;
      if (topBtn) topBtn.classList.toggle('show', doc.scrollTop > 500);
    };
    document.addEventListener('scroll', onScroll, {passive:true}); onScroll();
    if (topBtn) topBtn.addEventListener('click', () => window.scrollTo({top:0, behavior: reduce?'auto':'smooth'}));
  } catch {}
  ...
  try {
    ...
    const openModal = (title, media) => {
      if (!modalRoot || !slides) return;
      if (titleEl) titleEl.textContent = title || "Preview";
      slides.innerHTML = '';
      media.forEach(m => {
        let node;
        if ((m.type||'').includes('video') || /\.mp4$/i.test(m.src||'')) {
    ...
    const closeModal = () => {
      ...
      if (slides) qsa('video', slides).forEach(v => { try { v.pause(); } catch{} });
    };
    ...
    qsa('.project-card, .portfolio-item').forEach(card => {
      const titleNode = qs('h3, .title, .card-title', card);
  ...
  try {
    ...
    if (palette) {
      ...
      qsa('h1, h2, h3, nav a[href^="#"], section[id]').forEach(el => {
        ...
        else { const sec = el.closest ? el.closest('section') : null; if (sec) { if (!sec.id) sec.id = txt.toLowerCase().replace(/\s+/g,'-'); href = `#${sec.id}`; } }
        index.push([txt, href]);
      });
      ...
      document.addEventListener('keydown', (e)=> {
        ...
        if (k==='arrowdown'){ e.preventDefault(); sel = Math.min(sel+1, results.children.length-1); render(input.value); }
        if (k==='arrowup'){ e.preventDefault(); sel = Math.max(sel-1, 0); render(input.value); }
        if (k==='enter'){ e.preventDefault(); const a = results.querySelector('.command-item.active'); if (a){ const href = a.dataset.href||''; close(); if (href.startsWith('#')) { const t = document.querySelector(href); if (t) t.scrollIntoView({behavior: reduce?'auto':'smooth'}); } else window.open(href,'_blank'); } }
      });
      input.addEventListener('input', ()=> { sel=0; render(input.value); });
      results.addEventListener('click', (e)=> {
        const el = e.target && e.target.closest ? e.target.closest('.command-item') : null; if (!el) return;
        const href = el.dataset.href||''; close(); if (href.startsWith('#')) { const t = document.querySelector(href); if (t) t.scrollIntoView({behavior: reduce?'auto':'smooth'}); } else window.open(href,'_blank');
      });
    }
  } catch {}
